...
/**
 * Finds a specified vertex search in the graph and returns it
 * 
 * @param search The vertex being searched for
 * @return returns true if the vertex is found, and false if it is not found 
 */
bool Graph::BFS(Vertex search){
    //get vector of vertices and edges and create a map to keep track of visited vertices
    vector<Vertex> vertices = getVertices();
    vector<Edge> edges = getEdges();
    map<Vertex, bool> visited;
    //initialize all of the vertices to false in the visited map
    for(Vertex &v : vertices){
        visited.insert(std::pair<Vertex, bool>(v, false));
    }
    //for every vertex (even unconnected ones) return true if it is found in the graph and false if it is not
    for(Vertex &v : vertices){
        if(visited[v] == false){
            if(BFS(search, v, visited)) 
                return true;
        }
    }
    return false;
...
/**
 * helper function for BFS function above
 */
bool Graph::BFS(Vertex search, Vertex compVert, map<Vertex, bool> &visited){
    queue<Vertex> queue;
    //set the first vertex being visited to true and return if that vertex is the one being searched for, otherwise push into queue for BFS search
    visited[compVert] = true;
    if(compVert == search)
        return true;
    queue.push(compVert);

    while(!queue.empty()){
        //pop off the front of the queue and get a vector of all adjacent vertices
        Vertex v = queue.front();
        queue.pop();
        vector<Vertex> adjacent = getAdjacent(v);
        //for every adjacent vertex, if not visited previously, check with search vertex, and if not set labels accordingly
...
std::map<Vertex, std::pair<int, Vertex>> Graph::dijkstras(Vertex source) {
    vector<Vertex> verticies = getVertices();
    int size = verticies.size(); 

    // Map containing all verticies as keys, and a pair containing the distance from the source
    // to the vertex and the preceding vertex on the shortest path to the source
    std::map<Vertex, std::pair<int, Vertex>> distances; 

    // Visited map to keep track of which verticies we've visisted 
    std::map<Vertex, bool> visited; 

    // Initialize default values for both maps
    for (int n = 0; n < size; n++) {
        distances[verticies[n]] = std::pair<int, Vertex>(INT_MAX, "-");
        visited[verticies[n]] = false; 
    }

    if (distances.find(source) == distances.end()) {
        // If the source vertex does not exist in graph, return an empty map 
        return std::map<Vertex, std::pair<int, Vertex>>(); 
    } else {
        // Set the distance to the source destination to 0 
        distances[source].first = 0; 
    }

    int numVisited = 0; 
    Vertex currVisit = source; 
    bool unvisitedVerticies = true; 
    while (numVisited < size && unvisitedVerticies) {
        vector<Vertex> adjacentVerticies = getAdjacent(currVisit);
        int minDist = INT_MAX;
        Vertex nextVertex = "";
        unvisitedVerticies = false; 
        for (Vertex v : adjacentVerticies) {
            int weight = getEdgeWeight(currVisit, v);
            int totalDist = distances[currVisit].first + weight;
            // If the path from the current vertex to the adjacent vertex is smaller than the exisiting path,
            // update the vertex's current shortest path, and the previous vertex      
            if (totalDist < distances[v].first) {
                distances[v].first = totalDist; 
                distances[v].second = currVisit; 
            }
            // Vertex with the shortest edge is the next vertex to visit 
            if (totalDist < minDist && !visited[v]) { 
                unvisitedVerticies = true; 
                minDist = totalDist;
                nextVertex = v; 
            }
        }
        // Mark the current vertex as visited 
        visited[currVisit] = true; 
        currVisit = nextVertex; 
        numVisited++; 
    }
    return distances; 
}

vector<Vertex> Graph::shortestPath(Vertex source, Vertex dest) {
    // Distance map from djkstra's algorithm 
    std::map<Vertex, std::pair<int, Vertex>> referenceMap = dijkstras(source);
    Vertex currVertex = dest; 
    vector<Vertex> path; 
    // While the current vertex is not the starting vertex, keep backtracking through the map
    while (currVertex != "-") {
        path.insert(path.begin(), currVertex); 
        // Get the preceding vertex along the shortest path to the source node. 
        currVertex = referenceMap[currVertex].second; 
    }
    return path; 
}


std::vector<Vertex> Graph::landmarkPath(Vertex source, Vertex stop, Vertex dest) {
    vector<Vertex> path; 

    // Find the shortest distances from the stop to the destination 
    std::map<Vertex, std::pair<int, Vertex>> referenceMap = dijkstras(stop);
    Vertex currVertex = dest; 
    while (currVertex != "-") {
        path.insert(path.begin(), currVertex); 
        currVertex = referenceMap[currVertex].second; 
    ...
    // Find the shortest distances from the source to the stop 
    std::map<Vertex, std::pair<int, Vertex>> referenceMap2nd = dijkstras(source);
    currVertex = stop; 
    while (currVertex != "-") {
        if(currVertex != stop) path.insert(path.begin(), currVertex); 
        currVertex = referenceMap2nd[currVertex].second; 
    }
