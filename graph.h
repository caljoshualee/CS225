...
public:
    ...
    /**
     * Calculates the shortest distance from a given source to every other vertex on the graph using Dijskstras algorithm.
     * Also allows for backtracking to find the shortest path from the source to every other vertex 
     * 
     * @param source The starting vertex 
     * @return a map with each vertex in the graph as the keys, and a pair containing the shortest distance from the source to that vertex, and the previous vertex on the shortest path to reach that target vertex. 
     */
    std::map<Vertex, std::pair<int, Vertex>> dijkstras(Vertex source);

    /**
     * Finds the shortest path from a given source vertex to a given destination vertex by 
     * backtracking through the map returned by Dijkstra's algorithm
     * 
     * @param source The starting vertex
     * @param dest   The destination vertex
     * @return       A vector containing the shortest path from the source to the destination 
     */
    vector<Vertex> shortestPath(Vertex source, Vertex dest); 

    /**
     * Calculates the shortest distance from a given source to a stop (AKA landmark), and then finally a destination. 
     * Uses dijkstras algorithim twice.  
     * 
     * @param source The starting vertex  
     * @param stop   The "landmark" vertex
     * @param dest   The destination vertex
     * @return       A vector containing the shortest path from the source to the stop and then to the destination  
     */
    std::vector<Vertex> landmarkPath(Vertex source, Vertex stop, Vertex dest);
     ...
     */
    bool BFS(Vertex search);

    /**
     * helper function for BFS function above
     */
    bool BFS(Vertex search, Vertex compVert, std::map<Vertex, bool> &visited);
